...
var environment = config['api-proxy-deploy'].env;
...
console.log('&$api-proxy-deploy api: [' +  domains[environment] + ', ' + config['api-proxy-deploy'].basepath + ']');

BeforeAll("BeforeFeatures", function(next) {
    if (apps[0].name) {
        console.log ("Getting Creds from DevAppsKeys.json");
        getCreds(config['api-proxy-deploy'].app, config['api-proxy-deploy'].product);
    } else
    {
...
Before(function(scenario, callback) {
    ...
    this.apickli = new apickli.Apickli(tokens[0],
        tokens[1]+ config['api-proxy-deploy'].basepath,
        './test/integration/features/fixtures/');
    callback();
});
